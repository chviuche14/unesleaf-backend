...
/**
 * Guarda un registro de búsqueda
 * Body: { lng:number, lat:number, texto?:string, tipo?:'ciudad'|'unesco' }
 */
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    const usuarioIdRaw = req.user?.id ?? req.user?.userId ?? req.user?.usuario_id;
...
router.get('/', authenticateToken, async (req, res) => {
  try {
    const usuarioId = Number(req.user?.id ?? req.user?.userId ?? req.user?.usuario_id);
